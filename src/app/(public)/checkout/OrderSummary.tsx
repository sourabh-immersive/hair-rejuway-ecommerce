...
import { removeFromCartAsync, removeItemLocally } from "@/lib/features/cart/cartBSlice";
...
const OrderSummary: React.FC<OrderSummaryProps> = ({ items, totalPrice, totalItems, isLoading, userStatus, isSubmitting }) => {
  const dispatch = useAppDispatch();

  const renderProduct = useCallback((item: any, index: number) => {
    const { product_image, prices, name, key, product_id, variation, quantity, product_qty } = item;
    ...
    return (
      <div key={index} className="flex py-4">
        <div className="relative h-24 w-20 flex-shrink-0 overflow-hidden rounded-xl bg-slate-100">
          <Image fill src={product_image || ""} alt={name || "product"} className="object-contain object-center" />
        ...
        <div className="ml-4 flex-1 flex flex-col">
          ...
          <p className="text-sm text-slate-600">
            {variation?.map((att: any, i: number) => (
              <span key={i}>
                {att.attribute_title}: {att.attribute_value}{i < variation.length - 1 && ", "}
              </span>
            ))}
          </p>
          <div className="flex justify-between mt-auto text-sm">
            <span className="text-gray-500">Qty {qty}</span>
            <Prices price={prices?.regular_price} salePrice={prices?.sale_price} />
      ...
      <div className="mt-6 pt-4 text-sm text-slate-500 border-t border-slate-200/70">
        ...
        <div className="mt-4">
          ...
          <div className="flex mt-1.5">
            ...
            <button className="ml-2 px-4 py-2 bg-neutral-200/70 hover:bg-neutral-100 rounded-2xl border border-neutral-200">Apply</button>
          </div>
        </div>
        <ButtonPrimary className="mt-6 w-full" type="submit" disabled={isSubmitting || !items.length}>
